...
fn test_identifier() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::identifier, "id")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
    assert_eq!( pair.as_str(), "id" );

    let pair = Grammar::parse(Rule::identifier, "id_1")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_number() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::number, "13")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_string() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::string, "\"str\"")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_comparison_op() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::comparison_op, "<=")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_logical_op() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::logical_op, "AND")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_aggregate_func() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::aggregate_func, "MAX")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_order_type() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::order_type, "DESC")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_agg_field() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::agg_field, "COUNT(id)")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
    assert_eq!( pair.as_str(), "COUNT(id)" );

    let pair = Grammar::parse(Rule::agg_field, "id")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
    assert_eq!( pair.as_str(), "id" );
...
fn test_where_condition() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::where_condition, "n < 56")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
    assert_eq!( pair.as_str(), "n < 56" );

    let pair = Grammar::parse(Rule::where_condition, "city = \"Kyiv\"")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_join_clause() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::join_clause, "JOIN table ON id1 = id2")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_where_clause() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::where_clause, "WHERE id = 45")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
    assert_eq!( pair.as_str(), "WHERE id = 45" );

    let pair = Grammar::parse(Rule::where_clause, "WHERE id = 45 AND a > 56")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
    assert_eq!( pair.as_str(), "WHERE id = 45 AND a > 56" );
    
    let pair = Grammar::parse(Rule::where_clause, "WHERE id = 45 AND a > 56 OR b = \"str\"")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_order_by_clause() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::order_by_clause, "ORDER BY name")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
    assert_eq!( pair.as_str(), "ORDER BY name" );

    let pair = Grammar::parse(Rule::order_by_clause, "ORDER BY name DESC")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
...
fn test_limit_clause() -> anyhow::Result< () > {
    let pair = Grammar::parse(Rule::limit_clause, "LIMIT 10")?.next().ok_or_else( || anyhow!( "no pair" ) )?;
